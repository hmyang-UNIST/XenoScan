    ...
    for i_folder in folders:
        if i_folder[0] == '0':
            ...
            with tqdm(total=n_target) as pbar:
                for _ in tqdm(pool.imap_unordered(func, pathsidx)):
                    pbar.update()
            pool.close()
            pool.join()
...
def generate_ds_factor(src_path,dst_path,factor,dst_size=256,ballanced=True):
    ...
    for i_label in labels:
        ...
        with tqdm(total=i_n_target) as pbar:
            for _ in tqdm(pool.imap_unordered(func, range(n_target))):
                pbar.update()
        pool.close()
        pool.join()
    return None